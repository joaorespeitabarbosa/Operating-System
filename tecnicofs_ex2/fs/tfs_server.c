...
int main(int argc, char **argv)
{
    ...
    while (i == 1)
    {
        ...
        if (read(f_server, &buffer, sizeof(buffer)) == -1)
        {
            return -1;
        }
        ...
        case TFS_OP_CODE_MOUNT:
            ...
            if (write(f_client, &session_id, sizeof(session_id)) == -1)
            {
                return -1;
            }
            break;
        ...
        case TFS_OP_CODE_OPEN:
            ...
            session_id = atoi(buffer + sizeof(char));
            f_client = open(sessions[session_id], O_WRONLY);
            if (write(f_client, &f_handle, sizeof(f_handle)) == -1)
            {
                return -1;
            }
            break;
        ...
        case TFS_OP_CODE_CLOSE:
            ...
            f_client = open(sessions[session_id], O_WRONLY);
            if (write(f_client, &result, sizeof(result)) == -1)
            {
                return -1;
            }
            break;
        ...
        case TFS_OP_CODE_WRITE:
            ...
            session_id = atoi(aux_code);
            f_client = open(sessions[session_id], O_WRONLY);
            if (write(f_client, &result_ld, sizeof(result_ld)) == -1)
            {
                return -1;
            }
            free(content);
        ...
        case TFS_OP_CODE_READ:
            ...
            session_id = atoi(aux_code);
            f_client = open(sessions[session_id], O_WRONLY);
            if (write(f_client, &result_lui, sizeof(result_lui)) == -1)
            {
                return -1;
            }
            if (write(f_client, content, result_lui * sizeof(char)) == -1)
            {
                return -1;
            }
            free(content);
